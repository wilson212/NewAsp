...
{
    $nick = $_POST['nick'];
    $isBot = (isset($_POST['ai']) && is_numeric($_GET['ai'])) ? $_POST['ai'] : 0;
} 
else
{
    $nick = (isset($_GET['nick'])) ? $_GET['nick'] : '';
    $isBot = (isset($_GET['ai']) && is_numeric($_GET['ai'])) ? $_GET['ai'] : 0;
}
...
if(isset($nick) && $nick != '') 
{
    ...
    else 
    {
        ...
        $query = "INSERT `player` (id, name, joined, isbot) SELECT LEAST(IFNULL(MIN(id),". $cfg->get('game_default_pid') ."), ". 
            $cfg->get('game_default_pid') .")-1 AS id, '". quote_smart($nick) ."' AS name, " . time() . " AS joined, ".
