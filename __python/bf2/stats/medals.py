...
if ( running_mod.lower() == 'mods/xpack' ):
	if (medals_custom_data != ''):
		try:
			exec 'from bf2.stats.medal_data_xpack_' + str(medals_custom_data) + ' import *'
		except:
			print "Custom Medals Xpack Data Pack (%s) *NOT* found or is corrupt!" % (str(medals_custom_data))
			# Use defualt medals_data instead
			from bf2.stats.medal_data_xpack import *
		else:
			print "Custom Medals Xpack Data Pack (%s) loaded." % (str(medals_custom_data))
	else:
		from bf2.stats.medal_data_xpack import *
else:
	if (medals_custom_data != ''):
